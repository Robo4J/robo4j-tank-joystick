...
public class JoystickEventProducer implements EventHandler<MouseEvent> {
	...
	public JoystickEventProducer(Node node, DoubleProperty povCenterXProperty, DoubleProperty povCenterYProperty,
			DoubleProperty povCenterXLayoutProperty, DoubleProperty povCenterYLayoutProperty,
	...
	private int determinLevel(double x, double y) {
		...
		return levels.entrySet().stream().filter(e -> MoveCalculatorUtil.isInsideCircleArea(e.getValue().get(), x, y))
				.max((l, r) -> r.getKey().compareTo(l.getKey())).get().getKey();
